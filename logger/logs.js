...
class Logger {
  ...
  info(...args) { console.log(this.formatMessage(logLevels.INFO, ...args)); }
  success(...args) { console.log(this.formatMessage(logLevels.SUCCESS, ...args)); }
  warn(...args) { console.log(this.formatMessage(logLevels.WARN, ...args)); }
  error(...args) { console.log(this.formatMessage(logLevels.ERROR, ...args)); }
  debug(...args) { console.log(this.formatMessage(logLevels.DEBUG, ...args)); }
  ...
  activity(type, user, chat, command = null, status = null) {
    ...
    if (chat?.type !== 'private') { activityMsg += ` in ${c.bright(chatName)}`; }
    if (command) { activityMsg += ` used ${c.yellow(command)}`; }
    if (status === 'success') { activityMsg += ` ${c.green('✓')}`; } 
    else if (status === 'failed') { activityMsg += ` ${c.red('✗')}`; }
  ...
  commandExecution(user, chat, command, success, error = null) {
    ...
    if (!success && error) { this.error(`Command ${command} failed: ${error}`); }
  }
  ...
  separator(char = '═', color = 'dim') { console.log(c[color](char.repeat(70))); }
