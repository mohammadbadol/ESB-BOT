// bot/server/autoUptime.js → BADOL/autoUptime.js - v2.0 Custom Path Fixed
...
function getConf() {
  ...
  return {
    ...
    port: cfg.serverUptime?.port || cfg.dashboard?.port || cfg.dashBoard?.port || 3001
  };
}
...
if (!conf.autoUptime || !conf.autoUptime.enable) {
	return;
}

const PORT = conf.serverUptime?.port || 3001;

let myUrl = conf.autoUptime.url;
...
setTimeout(async function autoUptime() {
	...
	global.timeOutUptime = setTimeout(autoUptime, (conf.autoUptime.timeInterval || 180) * 1000);
}, (conf.autoUptime.timeInterval || 180) * 1000);
